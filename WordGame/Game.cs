...
namespace WordGame
{
    ...
    "zoril", "zowie", "zymes" };// list word
        public void WordleGame()
        {
            ...
            {
                Console.WriteLine("-----------");
                Console.Write($"Please enter your guess - attempt " + y + ":");
                input = Console.ReadLine();
                int totalCorrect = 0; // stage 7:initialize the variable that counts the number of characters in the correct position
                int wrongSpot = 0;//stage 7 :initialize the variable that counts the number of characters in the wrong place
                string inputCompare = input;
                string remainLetterInWord = wordToGuess;

                if (input.Length == 5)
                {
                    //stage 11: compare the input word with the word list in the initializer
                    if (WORD_LIST.Contains(input))
                    {
                        ...
                        for (int i =0; i < remainLetterInWord.Length; i++)
                        {
                            ...
                            //stage 6: check for characters that don't appear
                            else if (remainLetterInWord.Contains(inputCompare[i]) == false)
                            {
